...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$window', '$timeout', '$http', '$document', '$interval',
    function($scope, FabricWindow, NBDDataFactory, filterFontFilter, filterArtFilter, $window, $timeout, $http, $document, $interval){
        ...
        $scope.showClipartByCat = function( cat) {
            ...
            if($scope.resource.clipart.data.cat_data[cat.id].length > 0) {
                $scope.resource.clipartByCat.data = $scope.resource.clipart.data.cat_data[cat.id];
                $scope.resource.clipartByCat.filteredArts = $scope.resource.clipart.data.cat_data[cat.id];
                $scope.resource.clipartByCat.total = $scope.resource.clipart.data.cat_data[cat.id].length;
                $scope.resource.clipartByCat.nextPage = $scope.resource.clipart.nextPage[cat.id];
                $timeout(function(){
            ...
            switch(type){
                ...
                case 'myClipart':
                    ...
                    function getResources(cat) {
                        var url = apiPre + 'clipart-storages/' + cat.id + '/cliparts?limit=' + $scope.resource[type].filter.perPage;
                        if(type === 'myClipart') {
                            url = apiPre + 'clipart-storages/' + cat.id + '/cliparts-default?limit=' + $scope.resource[type].filter.perPage;
                        }
                        return $http({
                            method: "GET",
                            url: url,
