...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$window', '$timeout', '$http', '$document', '$interval',
    function($scope, FabricWindow, NBDDataFactory, filterFontFilter, filterArtFilter, $window, $timeout, $http, $document, $interval){
        ...
        $scope.getResource = function(type, container, callback){
            ...
            var headers = '';
            if(type === 'clipart') {
                url = apiPre + 'clipart-storages/default?limit=5';
                headers = apiAuthBearer;
            } else if( type === 'myClipart') {
                url = apiPre + 'clipart-storages';
                headers = apiAuthBearer;
            }
            $http({
                method: "GET",
        ...
        $scope.onClipartPrev = function(catId) {
            if(!catId) return;
            var width = $('.printcart-clipart-storage-' + catId).width();
            var container = '.printcart-clipart-storage-' + catId + ' .tab-scroll';
            var scrollX = jQuery(container).prop("scrollLeft");
            jQuery(container).stop().animate({
                scrollLeft: scrollX - width,
            }, 100);
        }
        ...
        $scope.afterGetResource = function(type){
            $scope.resource[type].data.cat_data = [];
            switch(type){
                case 'clipart':
                case 'myClipart':
                    ...
                    Promise.all(promises).then(function(response) {
                        if(response.length > 0) {
                            ...
                            angular.forEach(response, function(res, key) {
                                if(angular.isDefined(res.data) ) {
                                    ...
                                    if( $scope.resource[type].data.cat.length ){
                                        ...
                                        $scope.resource[type].filter.total = res.data.data.length;
                                    }else{
                                        $scope.resource[type].filter.currentCat = -1;
                                        $scope.resource[type].filter.currentPage = 0;
                                        $scope.resource[type].filter.total = 0;
                                    }
                                    // jQuery('.printcart-clipart-storage-' + _cat.id + ' .loading-photo').hide();
                                }
                            })
                            jQuery('#tab-svg').removeClass('nbd-onload');
                            $timeout(function(){
                                $('.printcart-clipart .tab-scroll').perfectScrollbar();
                                var scrollEl = $('.printcart-clipart .tab-scroll');
                                scrollEl.on('ps-scroll-x', function(){
                                    // if(scrollEl.prop("clientWidth") != scrollEl.prop("scrollWidth") && ((scrollEl.prop("scrollLeft") + scrollEl.prop("clientWidth") - scrollEl.prop("scrollWidth") + _offset) > 0) ){
                                    //     scope.action({container: scope.container, type: scope.type});
                                    // };
                                    // if( _elInfo.length ){
                                    //     scrollEl.prop("scrollTop") > 1500 && _elInfo.addClass('slideInDown animated show') || _elInfo.removeClass('slideInDown animated show');
                                    // }
                                });
                            });
                        } else {
                            if($scope.resource[type].data.cat.length > 0) {
                                angular.forEach($scope.resource[type].data.cat, function(cat, key) {
                                    // jQuery('.printcart-clipart-storage-' + cat.id + ' .loading-photo').hide();
                                })
                            }
                        }
                    });
