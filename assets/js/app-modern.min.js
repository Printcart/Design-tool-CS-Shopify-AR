                        ...
                        if(response.length > 0) {
                            ...
                            angular.forEach(response, function(res, key) {
                                if(angular.isDefined(res.data) ) {
                                    $scope.resource[type].data.cat_data[cat.id] = res.data.data;
                                    var cat = $scope.resource[type].data.cat[key];
                                    if(response.data.links.next) {
                                        $scope.resource[type].nextPage[cat.id] = response.data.links.next+ '&limit=' + $scope.resource[type].filter.perPage;
                                    } else {
                                        $scope.resource[type].nextPage[cat.id] = null;
                                    }
                                    var firstEl = {
                                        name: cat.name,
                                        id: cat.id,
    ...
    return {
        ...
        link: function( scope, element, attrs ) {
            $timeout(function() {
                ...
                el.on('ps-scroll-x', function(){
                    if(el.prop("clientWidth") != el.prop("scrollWidth") && ((el.prop("scrollLeft") + el.prop("clientWidth") - el.prop("scrollWidth") + offset) > 0) ){
                        scope.action({container: scope.container, type: scope.type});
                    };
                    if( elInfo.length ){
                        el.prop("scrollTop") > 1500 && elInfo.addClass('slideInDown animated show') || elInfo.removeClass('slideInDown animated show');
