...
nbdApp.controller('designCtrl', ['$scope', '$sce', 'FabricWindow', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$window', '$timeout', '$http', '$document', '$interval',
    function($scope, $sce, FabricWindow, NBDDataFactory, filterFontFilter, filterArtFilter, $window, $timeout, $http, $document, $interval){
        ...
        $scope.initSettings = function() {
            ...
            $scope.printing_options = {
                color: false,
                size: false,
            };
            if(productSettings?.metadata?.printingOptions?.options?.color?.attributes?.length > 0) {
                $scope.printing_options.color = productSettings?.metadata?.printingOptions?.options?.color;
            }
            if(productSettings?.metadata?.printingOptions?.options?.size?.attributes?.length > 0) {
                $scope.printing_options.size = productSettings?.metadata?.printingOptions?.options?.size;
            }
            $scope.resource = {
                ...
                variations: {data: [], perPage: 10, currentPage:1, total: 0, onload: false, init: true, nextPage: ''},
                ...
                clipartByCat: { data: [], filteredArts : [], nextPage: '', perPage: 20, currentPage:1, total: 0, onload: false, init: true},
                templates: { data: [], filtereTemps: [], next: '', perPage: 10, currentPage:1, total: 0, onload: false, init: true},
            ...
            $scope.preview_3d_url = productSettings?.metadata?.preview3d?.url ? productSettings.metadata.preview3d.url : './assets/image/Mug.glb';
            if(defaultFonts) {
            ...
            // if(side_id && (task == "create_template" || task == "edit_template")) {
                ...
                $scope.getVariantionByProduct(product_id, parentId); // get Products first load.
            // }
        ...
        $scope.changePrintingOptions = function() {}
        ...
        $scope.getVariantionByProduct = function(productId, parentId){
            var id = parentId ? parentId : productId;
            if(!id) return;
            ...
            if($scope.resource.variations.data.length > 0) {
                $scope.resource.variations.currentPage = 1;
                $scope.resource.variations.filtereProducts = $scope.resource.variations.data.slice(0, $scope.resource.variations.perPage);
                $scope.updateApp();
            } else {
                $scope.resource.variations.onload = true;
                var container = '#tab-product';
                jQuery(container + ' .loading-photo').show();
                var url = apiPre + 'products/' + id + '/variants?limit=' + $scope.resource.variations.perPage;
                var headers = apiAuthBearer;
                headers['Content-Type'] = undefined;
                $http({
                    method: "GET",
                    ...
                    var _data = response.data;
                    if(angular.isDefined(_data.data)) {
                        $scope.resource.variations.data = _data.data;
                        $scope.resource.variations.filtereProducts = _data.data;
                        $scope.resource.variations.total = _data.data.length;
                        $scope.resource.variations.onload = false;
                        $scope.resource.variations.next = _data.links.next ? _data.links.next + '&limit=' + $scope.resource.variations.perPage : '';
                        $scope.updateApp();
                    }
                    jQuery(container + ' .loading-photo').hide();
                }, function errorCallback(response) {
        ...
        $scope.showProducts = function(){
            ...
            if($scope.resource.products.data.length > 0) {
                ...
                $timeout(function() {
                    jQuery(".nbd-popup.popup-nbd-products .tab-scroll").stop().animate({
                        scrollTop: 0
                    }, 100);
                });
                $scope.updateApp();
            } else {
                jQuery('.nbd-popup.popup-nbd-products .printcart-loading').show();
                $scope.resource.products.onload = true;
                var url = apiPre + 'products?is_parent=true&limit=' + $scope.resource.products.perPage;
                var headers = apiAuthBearer;
                headers['Content-Type'] = undefined;
                $http({
                    method: "GET",
                    ...
                    if(angular.isDefined(_data.data)) {
                        ...
                        $scope.resource.products.next = _data.links.next ? _data.links.next + '&is_parent=true&limit=' + $scope.resource.products.perPage : '';
                        $scope.updateApp();
                    }
                    jQuery('.nbd-popup.popup-nbd-products .printcart-loading').hide();
                }, function errorCallback(response) {
                    jQuery('.nbd-popup.popup-nbd-products .printcart-loading').hide();
        ...
        $scope.scrollLoadMoreVariantionsProduct = function() {
            var currentPage = $scope.resource.variations.currentPage;

            if( $scope.resource.variations.onload ) return;

            if( currentPage * $scope.resource.variations.perPage >= $scope.resource.variations.total ) {
                if($scope.resource.variations.next) {
                    $scope.resource.variations.onload = true;
                    jQuery('#tab-product').addClass('nbd-onload');
                    jQuery('#tab-product .loading-photo').show();
                    var url = $scope.resource.variations.next;
                    var headers = apiAuthBearer;
                    headers['Content-Type'] = undefined;
                    $http({
                        method: "GET",
                        ...
                        $scope.resource.variations.currentPage = currentPage + 1;
                        $scope.resource.variations.data = $scope.resource.variations.data.concat(response.data.data);
                        $scope.resource.variations.total += response.data.data.length;                  
                        $scope.resource.variations.next = response.data.links.next ? response.data.links.next + '&limit=' + $scope.resource.variations.perPage : '';
                        $scope.resource.variations.filtereProducts = $scope.resource.variations.data;
                        $scope.updateApp();
                        $scope.resource.variations.onload = false;      
                        jQuery('#tab-product .loading-photo').hide();
                        jQuery('#tab-product').removeClass('nbd-onload');
                ...
                $scope.resource.variations.currentPage = currentPage + 1;
                var dataNext = $scope.resource.variations.data.slice( currentPage * $scope.resource.variations.perPage , (currentPage + 1) * $scope.resource.variations.perPage);
                $scope.resource.variations.filtereProducts = $scope.resource.variations.filtereProducts.concat(dataNext);
                $scope.updateApp(); 
            }
        }
        $scope.scrollLoadMoreProduct = function() {
            ...
            if( currentPage * $scope.resource.products.perPage >= $scope.resource.products.total ) {
                if($scope.resource.products.next) {
                    $scope.resource.products.onload = true;
                    jQuery('.nbd-popup.popup-nbd-products .product-load-more-wrap .loading-product').show();
                    jQuery('.nbd-popup.popup-nbd-products .product-load-more-wrap .tab-load-more').hide();
                    var url = $scope.resource.products.next;
                    var headers = apiAuthBearer;
                    headers['Content-Type'] = undefined;
                    $http({
                        method: "GET",
                        ...
                        if(response.status != 200 || angular.isUndefined(response.data.data) ) { 
                            jQuery('.nbd-popup.popup-nbd-products .product-load-more-wrap .loading-product').hide();
                            jQuery('.nbd-popup.popup-nbd-products .product-load-more-wrap .tab-load-more').show();
                            return;
                        }
                        ...
                        $scope.resource.products.next = response.data.links.next ? response.data.links.next + '&is_parent=true&limit=' + $scope.resource.products.perPage : '';
        ...
        $scope.changeProduct = function(id, isChangeProduct = true ) {
            if(!id) {
                return alert('Not found product.')
            }
            var newUrl = url.origin + '?api_key=' +  api_key_bearer + '&product_id=' + id;
            if(isChangeProduct) {
                newUrl += '&parent_id=' + id;
            } else {
        ...
        $scope.getResource = function(type, container, callback){
            ...
            if(type === 'clipart') {
                ...
                url = apiPre + 'clipart-storages?limit=20';
            }
            $http({
                method: "GET",
                url: url,
        ...
        $scope.isShowAllClipartWithoutCat = false;
        $scope.isSearchingClipart = true;
        $scope.currentClipartCat = {};
        $scope.currentCliparts = [];
        $scope.currentClipartType = 'myClipart';
        $scope.showClipartByCat = function( cat) {
            $scope.isSearchingClipart = false;
            if(angular.isUndefined(cat.id) || !cat.id || angular.isUndefined(cat.name) || !cat.name) return;
            $scope.currentClipartCat = cat;
            $scope.isShowAllClipartWithoutCat = true;
            if($scope.resource[$scope.currentClipartType].data.cat_data[cat.id].length > 0) {
                $scope.resource.clipartByCat.data = $scope.resource[$scope.currentClipartType].data.cat_data[cat.id];
                $scope.resource.clipartByCat.filteredArts = $scope.resource[$scope.currentClipartType].data.cat_data[cat.id];
                $scope.resource.clipartByCat.total = $scope.resource[$scope.currentClipartType].data.cat_data[cat.id].length;
                $scope.resource.clipartByCat.nextPage = $scope.resource[$scope.currentClipartType].nextPage[cat.id];
                $timeout(function(){
                    $scope.renderMasonryList('clipartByCat', '#tab-svg .clipart-wrap', '.clipart-item', '#tab-svg', true);
                });
            } else {
                var url = apiPre + 'clipart-storages/' + cat.id + '/cliparts?limit=' + $scope.resource.clipartByCat.perPage;
                $scope.resource.clipartByCat.onload = true;
                jQuery('#tab-svg').addClass('nbd-onload');
                $http({
                    method: "GET",
                    url: url,
                    ...
                    $scope.resource.clipartByCat.data = $scope.resource.clipartByCat.data.concat(response.data.data);;
                    $scope.resource.clipartByCat.filteredArts = $scope.resource.clipartByCat.data;
                    $scope.resource.clipartByCat.total = $scope.resource.clipartByCat.data.length;
                    $scope.resource.clipartByCat.nextPage = response.data.links.next;
                    $scope.resource.clipartByCat.onload = false;
                    jQuery('#tab-svg').removeClass('nbd-onload');
                    $timeout(function(){
                        $scope.renderMasonryList('clipartByCat', '#tab-svg .clipart-wrap.clipart-wrap-col', '.clipart-item', '#tab-svg', true);
                    });
                });
            }

        }
        $scope.backToTopTabClipart = function() {
            jQuery("#tab-svg .tab-scroll").stop().animate({
                scrollTop: 0
            }, 100);
        }
        $scope.hideClipartByCat = function() {
            $scope.isShowAllClipartWithoutCat = false;
            $timeout(function(){
                $('.printcart-clipart .tab-scroll').perfectScrollbar();
                $scope.backToTopTabClipart();
            });
        }
        $scope.enterSearchClipart = function(e, s) {
            if(e.which == 13) {
                $scope.searchClipart(s);
            }
        }
        $scope.searchClipart = function(s) {
            $scope.isShowAllClipartWithoutCat = true;
            $scope.isSearchingClipart = true;
            $scope.currentClipartCat = {
                id: 'search',
            ...
            if(!s) return;
            var url = apiPre + 'cliparts/default?s=' + s + '&limit=' + $scope.resource.clipartByCat.perPage;
            $scope.resource.clipartByCat.onload = true;
            jQuery('#tab-svg').addClass('nbd-onload');
            jQuery('#tab-svg .printcart-loading').show();
            $http({
                ...
                $scope.resource.clipartByCat.data = response.data.data;
                $scope.resource.clipartByCat.filteredArts = $scope.resource.clipartByCat.data;
                $scope.resource.clipartByCat.total = $scope.resource.clipartByCat.data.length;
                $scope.resource.clipartByCat.nextPage = response.data.links.next ? response.data.links.next + '&s=' + s + '&limit=' + $scope.resource.clipartByCat.perPage : '';
                $scope.resource.clipartByCat.onload = false;
                jQuery('#tab-svg').removeClass('nbd-onload');
                jQuery('#tab-svg .printcart-loading').hide();
                if(response.data.data.length > 0) {
                    $timeout(function(){
                        $scope.renderMasonryList('clipartByCat', '#tab-svg .clipart-wrap', '.clipart-item', '#tab-svg', false);
                        $('.printcart-clipart .tab-scroll').perfectScrollbar();
                        $scope.backToTopTabClipart();
                    });
                }
        ...
        $scope.scrollLoadMoreClipartByCat = function() {
            var type = 'clipartByCat';
            if( $scope.resource[type].onload) return;
            if( !$scope.isShowAllClipartWithoutCat ) return;
            if(!$scope.currentClipartCat || !$scope.currentClipartCat.id) return;
            if( !$scope.resource[type].nextPage ) return;
            $scope.resource[type].onload = true;
            jQuery('#tab-svg .loading-photo').show();
            jQuery('#tab-svg .printcat-clipart-by-cat .printcart-load-more').hide();
            $http({
                method: "GET",
                url: $scope.resource[type].nextPage,
                headers: apiAuthBearer,
            }).then(function successCallback(response){
                $scope.resource[type].nextPage = response.data.links.next;
                $scope.resource[type].data = $scope.resource[type].data.concat(response.data.data);
                $scope.resource[type].filteredArts = $scope.resource[type].data;
                $scope.resource[type].onload = false;
                jQuery('#tab-svg .loading-photo').hide();
                jQuery('#tab-svg .printcat-clipart-by-cat .printcart-load-more').show();
                $timeout(function(){
                    $scope.renderMasonryList('clipart', '#tab-svg .clipart-wrap', '.clipart-item', '#tab-svg', false);
                });
                $scope.updateApp();
            });
        }
...
,               parameters: JSON.stringify(item.toJSON()),
        ...
        $scope.showPopupAR = function() {
            jQuery('.nbd-popup.view-ar-on-mobile').nbShowPopup();
        }
        /* Close Deisgn tool */
